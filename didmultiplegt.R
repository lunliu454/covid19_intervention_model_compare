...
did_multiplegt_transform <- function(df, controls, trends_nonparam, period) {
  # Group by time and group, calculate the counter
  df <- df %>%
    group_by(.data$G, .data$T) %>%
    mutate(counter = n()) %>%
    summarize_all(mean) %>%
    mutate(periodG=ifelse(.data$T<=period[1],1,2)) %>%
    mutate(periodG=ifelse(.data$T>period[2],3,periodG))
  ...
  # df <- df %>%
  #   mutate(tag = rowSums(dplyr::across(.cols = !any_of("Y"), .fns = is.na))) %>%
                                      ...
                                      cluster, period)
{
  ...
  df <- did_multiplegt_transform(df, controls, !is.null(trends_nonparam), period)
...
residuals_control_once <- function(df, d, threshold_stable_treatment, diff_ctrl_names, trends_nonparam, trends_lin, periodg) {
  #control type 1
  #df_d <- df %>% filter( .data$DgroupLag == d ) #original
  #control type 2
  #df_d <- df #
  #control type 3
  ...
  for (ctrl in diff_ctrl_names){ #
    ...
    if(length(a)>0 & length(b)>0){ #
      if(max(a)!=0|min(a)!=0){ #
        if((max(a)-min(a))/(max(b)-min(b))<=0.5001){ #0.5 is arbitrary #
          #print(ctrl)
  ...
  # normal controls
  formula = paste(diff_ctrl_names, collapse = " + ")
  #don't add the Tfactor, since we have too many
  #if (do_count_tfactor) formula = paste(formula, " + Tfactor")
  formula = paste("diff_Y ~ ", formula, sep = "")
  counter = df_stable$counter
  #regress only on stable observations, by DgroupLag
  df_stable.lm <- lm(formula, data = df_stable, weights = counter)
  # f <- summary(df_stable.lm)$fstatistic
  # if (is.null(f)){
  #   return(df_d %>% mutate(residuals = .data$diff_Y)  %>% select(.data$T, .data$G, .data$residuals))
  # }
  # lm.p <- pf(f[1],f[2],f[3],lower.tail=F)
  # if (lm.p>0.05){
  #   return(df_d %>% mutate(residuals = .data$diff_Y)  %>% select(.data$T, .data$G, .data$residuals))
  # }
...
residuals_control <- function(df, controls, threshold_stable_treatment, trends_nonparam, trends_lin) {
  ...
  #     df, d, threshold_stable_treatment, diff_ctrl_names, trends_nonparam, trends_lin,counter_dynamic)
  #   df <- df %>%
  ...
  #   df <- df %>% #
  #     left_join(tmp_ret, by = c("T", "G")) %>% #
  #     mutate(diff_Y = residuals) %>% #
  #     select(-residuals) #
  
  #control type3: by period
  d = 0
  for (periodg in 1:3) {
    tmp_ret <- residuals_control_once(
      df, d, threshold_stable_treatment, diff_ctrl_names, trends_nonparam, trends_lin,periodg)
    df <- df %>%
      left_join(tmp_ret, by = c("T", "G")) %>%
...
did_multiplegt_core <- function(df, controls, threshold_stable_treatment, trends_nonparam, trends_lin,
                                counter_placebo = 0, counter_dynamic = 0, t_max = 0,direction) {
  effect = 0
  denom = 0
  total = 0
  switchers = 0
...
did_multiplegt_estim <- function(df, controls, placebo, dynamic, threshold_stable_treatment, trends_nonparam, trends_lin,direction) {
  t_max <- max(df$T)
  ret <- did_multiplegt_core(df, controls, threshold_stable_treatment,
                             trends_nonparam, trends_lin,counter_placebo = 0,
                             counter_dynamic = 0,
  ...
  if (placebo > 0) {
    ...
    for (i in 1:placebo) {
      ...
      ret_placebo_lag <- did_multiplegt_core(df_placebo_lag, controls, threshold_stable_treatment,
                                             ...
                                             counter_placebo = i,counter_dynamic = 0,
                                             t_max = t_max,direction)
...
did_multiplegt_bootstrap = function(df, controls, brep, trends_nonparam, trends_lin, has_cluster = FALSE,
  ...
  if (parallel) {
    ...
    #cl <- makeCluster(numCores,type="FORK") #windows
    matrix(unlist(mclapply(1:brep, callback, mc.cores = numCores)), nrow = brep, ncol = K, byrow = TRUE) #mac
  } else { #mac
    #   clusterExport(cl=cl, varlist=c("did_multiplegt_estim","did_multiplegt_core", #windows
    #                                  "residuals_control","fn_diff_rename","fn_ctrl_rename",
    #                                  "residuals_control_once","did_multiplegt_once",
    #                                  "dynamic_update_cond","fn_lead_rename",
    #                                  "prepare_dynamic_once","fn_diff_i_rename",
    #                                  "fn_lag_rename","placebo_update_cond",
    #                                  "prepare_placebo_once"#,
    #                                  # "has_cluster","controls","cluster","N_cluster",
    #                                  # "threshold_stable_treatment","recat_treatment",
    #                                  # "trends_nonparam","trends_lin"
    #   ))
    #   clusterEvalQ(cl=cl, library(dplyr,stringr)) #windows
    #   matrix(unlist(parallel::parLapply(cl,1:brep, callback)), nrow = brep, ncol = K, byrow = TRUE) #windows
    # } else { #windows
...
# parallel = TRUE
# direction = "both"
# period = c("2020-01-01","2020-12-30")
                ...
                period = c())
{
  ...
  eret = did_multiplegt_estim(df_preprocess, controls, placebo, dynamic, threshold_stable_treatment,
                              trends_nonparam, trends_lin,direction)
  
  if (brep == 0) {
    eret
  } else {
    if((length(eret)-placebo*2)/3<21){
    ...
    if (parallel) {
      numCores <- detectCores()
      #cl <- makeCluster(numCores,type="FORK") #windows
      bret = matrix(unlist(mclapply(1:brep, callback, mc.cores = numCores)), nrow = brep, ncol = K, byrow = TRUE) #mac
    } else { #mac
      bret = matrix(unlist(lapply(1:brep, callback)), nrow = brep, ncol = K, byrow = TRUE)
    }
    ...
    #                                 parallel = parallel,direction = "both")
    ...
    if (covariance) {
      ...
      if (!is.null(average_effect)) {
        ...
        # l <-c()#
        # while (k <= dynamic){#
        #   l <- c(l,min(eret[[3*k+3]],eret[[3*k+2]]-eret[[3*k+3]]))#
        #   k <- k+1#
        # }#
        # if (length(which(l<=5)) > 0){ #
        #   dynamic <- min(which(l<=5))-2 #
        # } #
        if (dynamic >= 0){
